#!/usr/local/bin/perl
#
#   NetSoup::XHTML::MenuMaker::Drivers::Owner.pm v00.00.01a 12042000
#
...
package NetSoup::XHTML::MenuMaker::Drivers::Owner;
use strict;
use NetSoup::XHTML::MenuMaker::Drivers;
use NetSoup::XML::DOM2::Core::Document;
use NetSoup::XML::File;
use NetSoup::XML::Parser;
@NetSoup::XHTML::MenuMaker::Drivers::Owner::ISA = qw( NetSoup::XHTML::MenuMaker::Drivers );
1;
...
sub initialise {
  ...
  # Example:
  #    method call
  my $Owner = shift;          # Get Owner object
  my %args    = @_;             # Get arguments
  $Owner->SUPER::initialise;
  return( $Owner );
}
...
sub build {
  ...
  my $outfile  = $args{Target}   || "MenuMaker_Owner.html";
  ...
  if( defined $Document ) {
    $Owner->traverse( Document => $Document,
...
sub traverse {
  ...
  for( my $i = 0 ; $i < $NodeList->nodeListLength() ; $i++ ) {
    ...
    if( $NodeList->item( Item => $i )->getAttribute( Name => "href" ) ) {
      ...
      my $owner = $NodeList->item( Item => $i )->getAttribute( Name => "owner" );
      if( $owner ) {
        my $OWNER = $Document->createTextNode( Data => " ($owner)" );
        $LI->appendChild( NewChild => $OWNER );
      }
    } else {
      my $TEXT = $Document->createTextNode( Data => $NodeList->item( Item => $i )->getAttribute( Name => "name" ) );
