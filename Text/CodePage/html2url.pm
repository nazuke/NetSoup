#!/usr/local/bin/perl
#
#   NetSoup::Text::CodePage::html2url.pm v00.00.01a 12042000
#
...
#       html2url    -  This method url-encodes Html data
#       url2html    -  This method converts url-encoded strings to Html data


package NetSoup::Text::CodePage::html2url;
use NetSoup::Text::CodePage;
@NetSoup::Text::CodePage::html2url::ISA = qw( NetSoup::Text::CodePage );
my %URL2HTML = ();
while ( <NetSoup::Text::CodePage::html2url::DATA> ) {
  my ( $hex, $html ) = ( m/^([^\t ]+)\s+([^\t ]+)\s/ );
  $URL2HTML{$hex}    = $html;
}
1;
...
sub html2url {
  # This method url-encodes Html data.
  # Calls:
  #    none
  # Parameters Required:
  #    object
  ...
  #    boolean
  # Example:
  #    $object->html2url( Data => \$data );
  my $object = shift;                     # Get object
  my %args   = @_;                        # Get arguments
  my $data   = $args{Data};               # Get unencoded data
  foreach ( keys %URL2HTML ) {            # Find code in hash
    $$data =~ s/\Q$URL2HTML{$_}\E/$_/gs;  # Perform global substitution
  }
  return(1);
}
...
sub url2html {
  # This method converts url-encoded strings to Html data.
  # Calls:
  #    none
  # Parameters Required:
  #    object
  ...
  #    boolean
  # Example:
  #    $object->url2html( Data => \$data );
  my $object = shift;                 # Get object
  my %args   = @_;                    # Get arguments
  my $data   = $args{Data};           # Get unencoded data
  foreach my $i ( keys %URL2HTML ) {  # Iterate over translation table
    $$data =~ s/$i/$URL2HTML{$i}/gs;  # Perform string substitution
  }
  return(1);
}
...
%99    &#153;    Trademark
%9A    &#154;    UNDEFINED
%9B    &#155;    Greater Than Sign
%9C    &#156;    Small oe Ligature
%9F    &#159;    Capital Y Umlaut
