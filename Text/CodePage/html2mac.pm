#!/usr/local/bin/perl
#
#   NetSoup::Text::CodePage::html2mac.pm v00.00.01a 12042000
#
...
#       mac2html    -  This method converts Mac-encoded characters to their equivalent Html Entity Tags
#       html2mac    -  This method converts Html Entity Tags to their equivalent Mac encodings


package NetSoup::Text::CodePage::html2mac;
use NetSoup::Core;
@NetSoup::Text::CodePage::html2mac::ISA = qw( NetSoup::Core );
my %BIDIRECT = ();
my %HTML2MAC = ();
my %MAC2HTML = ();
while ( <NetSoup::Text::CodePage::html2mac::DATA> ) {  # Read table form DATA segment
  my ( $hDir,
       $cDir,
       $html,
       $char ) = ( m/^                                 # Load up conversion tables
                   ...
                   ([^\t ]+)\t+                        # Mac character value
                   /x );
  $char            = chr($char);                       # Get Mac character from ascii value
  $BIDIRECT{$html} = $hDir;
  $BIDIRECT{$char} = $cDir;
  $HTML2MAC{$html} = $char;                            # Store entity tag direction
  $MAC2HTML{$char} = $html;                            # Store character direction
}
1;


sub mac2html {
  # This method converts Mac-encoded characters to their equivalent Html Entity Tags.
  # Calls:
  #    none
  # Parameters Required:
  #    object
  ...
  foreach my $i ( @chars ) {
    if( exists $BIDIRECT{$i} ) {
      if( $BIDIRECT{$i} =~ m/^\+$/ ) {  # Flagged for conversion ?
        $string .= $MAC2HTML{$i};       # Convert to entity tag
      } else {
        $string .= $i;                  # No conversion
      }
    } else {
      $string .= $i;                    # No conversion
    }
  }
  $$data = $string;                     # Put converted string into data reference
  return(1);
}


sub html2mac {
  # This method converts Html Entity Tags to their equivalent Mac encodings.
  # Calls:
  #    none
  # Parameters Required:
  #    object
  ...
  #    boolean
  # Example:
  #    $object->html2mac( Data => \$data );
  my $object = shift;                     # Get object
  my %args   = @_;                        # Get arguments
  my $data   = $args{Data};               # Get unencoded data
  foreach ( keys %HTML2MAC ) {            # Find code in hash
    $$data =~ s/\Q$_\E/$HTML2MAC{$_}/gs;  # Perform global substitution
  }
  return(1);
}
