#!/usr/local/bin/perl
#
#   NetSoup::Protocol::DRE::Query.pm v00.00.01a 12042000
#
...
package NetSoup::Protocol::DRE::Query;
use strict;
use Digest::MD5;
use NetSoup::Core;
use NetSoup::Files;
use NetSoup::Protocol::HTTP;
@NetSoup::Protocol::DRE::Query::ISA = qw( NetSoup::Core );
1;
...
sub query {
  ...
  if( $flag == 1 ) {
    ...
    my $Document = $HTTP->get( URL => "http://$Query->{Hostname}:$Query->{Port}/qmethod=$QMethod&querytext=$QueryText&qnum=$QNum&attachtoname=$Database&xoptions=$XOptions" );
    if( defined $Document ) {
